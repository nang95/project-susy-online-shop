          ...
          <div class="card-body px-0 pt-0 pb-2">
            <div class="table-responsive p-0">
              <table class="table align-items-center mb-0">
                <thead>
                  <tr>
                    ...
                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Tanggal Dari</th>
                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Tanggal Sampai</th>
                ...
                <tbody>
                  ...
                  <tr>
                    <td>{{ $data_event->nama }}</td>
                    <td>{{ date('d-m-Y'), strtotime($data_event->tanggal_dari) }}</td>
                    <td>{{ date('d-m-Y'), strtotime($data_event->tanggal_sampai) }}</td>
                    <td class="align-middle">
                      <a href="{{ route('admin.event.edit', $data_event->id) }}" style="padding-right: 7px" class="text-secondary font-weight-bold text-xs" data-toggle="tooltip" data-original-title="Edit user">
                        Edit
                      </a>
                      <a href="{{ route('admin.event.produk', $data_event->id) }}" class="text-secondary font-weight-bold text-xs" data-toggle="tooltip" data-original-title="Edit user">
                        Produk
                      </a>
                      <form onsubmit="deleteThis(event)" action="{{ route('admin.event.delete') }}" method="POST" style="display:inline-block">
              ...
              <div class="text-center">
                {{ $event->appends(['q_nama' => $q_nama])->links() }}
              </div>
            </div>
          </div>
        </div>
