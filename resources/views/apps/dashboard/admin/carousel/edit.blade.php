...
<div class="row">
    <div class="col-8">
        <div class="card mb-2">
          ...
          <div class="card-body">
            <form role="form" method="POST" action="{{ route('admin.produk.update') }}" enctype="multipart/form-data">
                @csrf @method('PUT')
                ...
                <div class="mb-3">
                  <textarea name="deskripsi" class="form-control" id="deskripsi" cols="10" rows="5">{{ $produk->deskripsi }}</textarea>
                </div>
                  ...
                  <select name="kategori_ids[]" class="form-control" id="kategori_ids" multiple>
                    ...
                    @foreach ($kategori as $item)
                        <option value="{{ $item->id }}"
                          @foreach ($produk->kategori()->get() as $key => $value)
                              @if ($value->id == $item->id)
                                selected
                              @endif
